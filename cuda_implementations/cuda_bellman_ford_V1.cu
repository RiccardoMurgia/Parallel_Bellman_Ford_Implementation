...
__device__ MinResult d_find_min_value(const int *array, int num_vertices, unsigned int tid){
    ...
    int end_index = start_index + num_vertices - 1;
...
__global__ void cuda_kernel_bellman_ford_v1(const int *d_dist, Graph *d_graph, const int *d_predecessor, int *d_new_dist, int *d_new_predecessor, int *d_candidate_dist){
...
extern "C" int cuda_bellman_ford_v1(Graph *graph, int source, int *dist, int threads_per_block){
    ...
    for (int i = 0; i < graph->num_vertices; i++){
        ...
        cuda_kernel_bellman_ford_v1<<<num_blocks, threads_per_block>>>(d_dist, d_graph, d_predecessor, d_new_dist, d_new_predecessor, d_candidate_dist);
        cudaDeviceSynchronize();

        cudaMemcpy(dist, d_new_dist, sizeof(int) * graph->num_vertices, cudaMemcpyDeviceToHost);
        cudaMemcpy(d_dist, dist, sizeof(int), cudaMemcpyHostToDevice);
        cudaMemcpy(predecessor, d_new_predecessor, sizeof(int) * graph->num_vertices, cudaMemcpyDeviceToHost);
        cudaMemcpy(d_predecessor, predecessor, sizeof(int), cudaMemcpyHostToDevice);
        cudaDeviceSynchronize();
